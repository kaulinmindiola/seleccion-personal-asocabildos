...
export default function Vacante() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("El usuario se ha postulado correctamente.");
      setTimeout(() => navigate("/"), 2500);

      if (res.data.credentials) setModalCred(res.data.credentials);
    } catch (err) {
      console.error(err);
      toast.error(err.response?.data?.msg || "Error al postular");
    }
  };
    ...
    <>
      ...
      <div className="pt-24 p-6">
        ...
        <input
          ...
          onChange={(e) =>
            setForm({ ...form, fechaExpedicion: e.target.value })
          }
          className="w-full border p-2 rounded"
          required
        />
        ...
        <CredentialsModal
          open={!!modalCred}
          credentials={modalCred}
          onClose={() => setModalCred(null)}
        />
      </div>
    </>
  );
